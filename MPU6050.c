...
uint8_t mpu6050_event(struct mpu6050 *self)
{
    ...
    self->activity.isDataReady = ((int_status >> 0) & 1);
...
void mpu6050_set_sleep_enabled(struct mpu6050 *self, uint8_t state)
{
    ...
    i2c_read_reg(&self->i2c, PWR_MGMT_1, &power_management_1, 1);

    if (state)
    {
        power_management_1 |= (1 << 6);
    }
    else
    {
        power_management_1 &= ~(1 << 6);
    ...
    uint8_t data[2] = {PWR_MGMT_1, power_management_1};
    i2c_write_blocking(self->i2c.instance, self->i2c.address, data, 2, false);
}
...
struct mpu6050_vectorf *mpu6050_get_gyroscope(struct mpu6050 *self)
{
    ...
    else if (self->rg.x != 0 || self->rg.y != 0 || self->rg.z != 0)
    {
        ...
        {
            if (fabsf(self->ng.x) < self->calibration_data.tg.x)
                self->ng.x = 0.0f;

            if (fabsf(self->ng.y) < self->calibration_data.tg.y)
                self->ng.y = 0.0f;

            if (fabsf(self->ng.z) < self->calibration_data.tg.z)
                self->ng.z = 0.0f;
        }
